...
def normalize_text(text: str) -> str:
    ...
    # 1. NFKC aggressively converts stylized/compatibility characters (e.g., 𝐏, 𝕾, 𝒮) to standard Latin (P, S, S)
    nfkc_form = unicodedata.normalize('NFKC', str(text))
    
    # 2. NFKD decomposes to separate combining characters (accents, marks)
    nfkd_form = unicodedata.normalize('NFKD', nfkc_form)
    
    # 3. Strip combining characters and lowercase the final output
    ascii_str = "".join([c for c in nfkd_form if not unicodedata.combining(c)])
    ascii_str = ascii_str.lower()
    
    # 4. Clean up any lingering punctuation to ensure a clean match
...
async def render_user_list_page(update: Update, context: ContextTypes.DEFAULT_TYPE, edit_message: bool = False):
    ...
    total_premium = sum(1 for u in all_db_users if u.get("expiry") and u["expiry"] > now)

    if filter_type == "premium":
        filtered_users = [u for u in all_db_users if u.get("expiry") and u["expiry"] > now]
    elif filter_type == "free":
        filtered_users = [u for u in all_db_users if not u.get("expiry") or u["expiry"] <= now]
    else:
                    ...
                    for line in lines:
                        if ":" in line:
                            if current_key:
                                ...
                                if current_key == "more channel link": updated_settings["more_channel_link"] = safe_url(val_str)
                                elif current_key == "video tutorial link": updated_settings["video_tutorial_link"] = safe_url(val_str)
                                elif current_key == "database channel id": updated_settings["database_channel_id"] = val_str
                                elif current_key == "start media url": updated_settings["start_media_url"] = safe_url(val_str)
                                elif current_key == "qr image url": updated_settings["qr_image_url"] = safe_url(val_str)
                                elif current_key == "verify banner url": updated_settings["verify_banner_url"] = safe_url(val_str)
                                elif current_key == "items per page": updated_settings["items_per_page"] = int(val_str) if val_str.isdigit() else 10
                                elif current_key == "about message": updated_settings["about_message"] = val_str
                                elif current_key == "1 month price": prices["1"] = val_str
                                elif current_key == "2 month price": prices["2"] = val_str
                                elif current_key == "3 month price": prices["3"] = val_str
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🎧 FM Stories App", web_app=WebAppInfo(url=webapp_url))]
    ]
...
<body>
    ...
    <div id="detail-view" class="secondary-view">
        ...
        <div class="content-card">
            ...
            <div class="action-buttons-container">
                <a id="btn-get-demos" href="#" target="_blank" class="action-btn btn-demo" onclick="handleMinimize()">🎧 Get Demo Episodes</a>
                <a id="btn-story-access" href="#" target="_blank" class="action-btn btn-access" onclick="handleMinimize()">🔓 Get Story Access</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Initialize display: show only first 10 items
        ...
        window.addEventListener('scroll', () => {
            if (isFilteringOrSearching) return; // Disable lazy loading pagination while user is searching/filtering
            ...
            if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - 100) {
                ...
                if (currentVisibleLimit < cards.length) {
                    ...
                    }, 300); // Small natural loading delay
                }
            }
        });
        ...
        function applyFilters() {
            ...
            // If a search or filter term is applied, show all matching elements regardless of batch limits
            ...
            for (let card of cards) {
                ...
                if (textMatch && genreMatch && accessMatch && categoryMatch && typeMatch && statusMatch) {
                    if (isFilteringOrSearching) {
                        ...
                        // Respect pagination limit when filters/search are cleared
        ...
        function openDetail(item, posterUrl) {
            ...
            let dpsLink = token10 ? `https://t.me/${botUsername}?start=dps_${token10}` : `https://t.me/${botUsername}`;
            let domoLink = token10 ? `https://t.me/${botUsername}?start=domo_${token10}` : `https://t.me/${botUsername}`;

            document.getElementById('btn-get-demos').href = domoLink;
            document.getElementById('btn-story-access').href = dpsLink;
        }
        ...
        function handleMinimize() {
            try {
                ...
                console.log("WebApp close fallback error:", e);
            }
        }
    </script>
</body>
